...
--FEATURE: CURSOR ON EMPTY CELLS
--Lets the cursor move onto and stop at roster cells that hold no character
--(MUGEN's moveoveremptyboxes). Disabled by default: with rosterpager active the
--cursor should skip empty cells and go straight to the next page instead.
selectplus.CursorOnEmpty = {
    enabled = false,
}
...
hook.add('start.f_selectScreen', 'selectplus', function()
	if selectplus.CursorOnEmpty.enabled then
		motif.select_info.moveoveremptyboxes = true
	end
